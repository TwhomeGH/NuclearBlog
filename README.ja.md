...
- **投稿を編集：** `@content/posts/`内のファイルを修正
- **特別ページをカスタマイズ：** `@content/spec/`内のファイルを編集
- **画像を追加：** 画像を`@assets/`または`public/`に配置
...
デプロイ前に、`@config.ts`の`siteURL`を更新してください。
...
**注意：**
この機能を使用するには、まず`@config.ts`でコメントシステムを有効にする必要があります。
...
- **アニメページ：** `@pages/anime.astro`でアニメリストを編集
- **友達ページ：** `@content/spec/friends.md`で友達データを編集
- **日記ページ：** `@pages/diary.astro`で瞬間を編集
- **アバウトページ：** `@content/spec/about.md`でコンテンツを編集
