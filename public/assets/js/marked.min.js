 ...
 * The code in this file is generated from files in ./@
 */
((g, f) => {
...
`),
							...
							((s[s.length - 1] = T),
								(r =
									r.substring(0, r.length - R.raw.length) +
									T.raw),
								(i =
									i.substring(0, i.length - R.text.length) +
									T.text));
							break;
						}
						if (k?.type === "list") {
...
`),
							...
							((s[s.length - 1] = T),
								(r =
									r.substring(0, r.length - k.raw.length) +
									T.raw),
								(i =
									i.substring(0, i.length - R.raw.length) +
									T.raw),
...
`,
								...
								if (
									...
									break;
								if (
									E.search(this.rules.other.nonSpaceChar) >=
										g ||
									!k.trim()
								)
									c +=
...
`),
						...
						i.loose ||
							(o
								? (i.loose = !0)
								: this.rules.other.doubleBlankLine.test(p) &&
									(o = !0));
						let T = null,
							ee;
						(this.options.gfm &&
					...
					for (let u = 0; u < i.items.length; u++)
						if (
							...
							const p = i.items[u].tokens.filter(
									(f) => f.type === "space",
								),
								c =
									p.length > 0 &&
									p.some((f) =>
										this.rules.other.anyLine.test(f.raw),
									);
							i.loose = c;
						}
					if (i.loose)
			...
			table(e) {
				...
				const n = Y(t[1]),
					r = t[2]
						...
						.split("|"),
					i = t[3]?.trim()
						? t[3].replace(this.rules.other.tableRowBlankLine, "")
								.split(`
`)
						: [],
					s = {
...
` + r.text),
			...
			inlineTokens(e, t = []) {
				...
				for (
					;
					(r = this.tokenizer.rules.inline.blockSkip.exec(n)) != null;
				)
					n =
						n.slice(0, r.index) +
						"[" +
		...
		var A = class {
				...
				return (
					e.forEach((n) => {
						...
						if (
							...
							n.extensions &&
								(n.extensions.forEach((i) => {
									...
									if ("renderer" in i) {
										...
										s
											? (t.renderers[i.name] = function (
													...o
												) {
													let l = i.renderer.apply(
														this,
														o,
													);
													return (
							...
							const i =
								this.defaults.renderer || new P(this.defaults);
						...
						if (n.tokenizer) {
							const i =
								this.defaults.tokenizer || new y(this.defaults);
							for (const s in n.tokenizer) {
								if (!(s in i))
									throw new Error(
										`tokenizer '${s}' does not exist`,
									);
			...
			parseMarkdown(e) {
				return (n, r) => {
					...
					if (s.async)
						return Promise.resolve(
							s.hooks ? s.hooks.preprocess(n) : n,
						)
							.then((p) => l(p, s))
							.then((p) =>
								s.hooks ? s.hooks.processAllTokens(p) : p,
							)
							.then((p) =>
								s.walkTokens
									? Promise.all(
											this.walkTokens(p, s.walkTokens),
										).then(() => p)
									: p,
							)
							.then((p) => u(p, s))
